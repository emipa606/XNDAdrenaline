...
public class Hediff_AdrenalineRush : Hediff_Adrenaline
{
    ...
    protected virtual float TargetSeverity
    {
        get
        {
            var modifiedThreatSignificance = EffectiveTotalThreatSignificance < 1
                ? EffectiveTotalThreatSignificance
                : Mathf.Sqrt(EffectiveTotalThreatSignificance);
            return ((modifiedThreatSignificance * Props.targetSeverityPerTotalThreatSignificance) +
                    (recentPainFelt * Props.targetSeverityPerRecentPainFelt)) *
                   AdrenalineTracker.AdrenalineProductionFactor;
        }
    }
    ...
    protected virtual float SeverityGainFactor
    {
        get
        {
            var baseFactor = Mathf.Sqrt(EffectiveTotalThreatSignificance) +
                             (recentPainFelt * Props.severityGainFactorOffsetPerRecentPainFelt);
            var adrenalineProduction = pawn.GetStatValue(A_StatDefOf.AdrenalineProduction);

            // Cube root adrenaline production when below 1 to speed up the rate at which the stat drops
            return adrenalineProduction < 1
                ? baseFactor * Mathf.Pow(adrenalineProduction, 1f / 3)
                : baseFactor * adrenalineProduction;
        }
    }
    ...
    protected override void UpdateSeverity()
    {
        ...
        {
            Severity -= Mathf.Min(Severity - TargetSeverity,
                Props.baseSeverityLossPerDay / GenDate.TicksPerDay * SeverityUpdateIntervalTicks *
                SeverityLossFactor);
        }
    ...
    public override string DebugString()
    {
        ...
        debugBuilder.AppendLine(
            $"total threat significance: {EffectiveTotalThreatSignificance} ({totalThreatSignificance})".Indented()
                .Indented());
